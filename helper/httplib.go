...
func (b *HTTPRequest) DoRequest() (resp *http.Response, err error) {
	...
	if trans == nil {
		...
		trans = &http.Transport{
			...
			Dial:                _TimeoutDialer(b.setting.ConnectTimeout, b.setting.ReadWriteTimeout),
		...
		if t, ok := trans.(*http.Transport); ok {
			...
			if t.Dial == nil {
				t.Dial = _TimeoutDialer(b.setting.ConnectTimeout, b.setting.ReadWriteTimeout)
			}
		}
	}
...
// _TimeoutDialer returns functions of connection dialer with timeout settings for http.Transport Dial field.
func _TimeoutDialer(cTimeout time.Duration, rwTimeout time.Duration) func(net, addr string) (c net.Conn, err error) {
...
// Read http body and rewrite body to http request
func _ReadNotDrain(r *http.Request) (content []byte, err error) {
