...
func (this *AliPay) VerifySignUrl(data url.Values) (ok bool) {
	var err error
	ok, err = verifySign(data, this.AliPayPublicKey)
	if err != nil {
		ok = false
	}
	return
...
func (this *AliPay) VerifySignUrlValue(data url.Values, sign string) (ok bool) {
	...
	var err error
	ok, err = verifyData([]byte(s), signType, sign, this.AliPayPublicKey)
	if err != nil {
		ok = false
	}
	return
